...
module GameData
	...
	CREATION_ITEMS_HASH = [
		...
		[[FIRE_FURNACE, INFERIOR_MAGIC_WORKBENCH], {
			{FIRE_CRYSTAL => 80000} => {INFERIOR_MAGIC_CRYSTAL => 4000},
		}],
		...
		[[WOOD_REFINERY, INFERIOR_MAGIC_WORKBENCH], {
			{WOOD => 40_0000} => {WOOD_CRYSTAL => 16000},
			{WOOD_CRYSTAL => 16000} => {INFERIOR_MAGIC_CRYSTAL => 4000},
		}],
		[[LARGE_FURNACE], {
		...
		[[WOOD_REFINERY, MAGIC_WORKBENCH], {
			{WOOD => 800_0000} => {WOOD_CRYSTAL => 320000},
			{WOOD_CRYSTAL => 320000} => {INFERIOR_MAGIC_CRYSTAL => 80000},
			{BLAZE_CRYSTAL => 1, INFERIOR_MAGIC_CRYSTAL => 400, WOOD_CRYSTAL => 12000} => {WOOD_SWORD => 200},
		}],
		[[METAL_REFINERY, MAGIC_WORKBENCH], {
			#{BLAZE_CRYSTAL => 5, INFERIOR_MAGIC_CRYSTAL => 1000, GOLD => ???, SILVER => ???}=>{INFERIOR_LIGHT_CRYSTAL => 400},
			# 大型炉隣につけて用地を足りなくさせるとか？溶けた金じゃないとだめだ！って
			#{INFERIOR_MAGIC_CRYSTAL => 2000, INFERIOR_LIGHT_CRYSTAL => 800} => {INFERIOR_LIGHT_SWORD => 200}, # なんかもうちょっとひねりを
			
			#元{INFERIOR_MAGIC_CRYSTAL => 200, GOLD => 400, SILVER => 1200, COPPER => 2000, IRON => 3000, FIRE_CRYSTAL => 2000} =>
			#      {INFERIOR_LIGHT_CRYSTAL => 20},
			#元{INFERIOR_MAGIC_CRYSTAL => 100, INFERIOR_LIGHT_CRYSTAL => 40} => {INFERIOR_LIGHT_SWORD => 10},
			
			# 金銀だけに絞るとか・・？=>これで行こう
			# 魔法結晶の割合高めるとか・・？=>調整めどい
			# 低数レシピを使うとか・・？=>めどい
		}],
		[[ANCIENT_REFINERY, MAGIC_WORKBENCH], {
			#元{INFERIOR_MAGIC_CRYSTAL => 400, INFERIOR_DARK_CRYSTAL => 80} => {INFERIOR_DARK_SWORD => 20},
			#元{INFERIOR_MAGIC_CRYSTAL => 1000, COAL => 100000, LIME => 100000} => {INFERIOR_DARK_CRYSTAL => 40},
		}],
		#   普通レシピ
		[[MAGIC_WORKBENCH, WOOD_REFINERY], {
